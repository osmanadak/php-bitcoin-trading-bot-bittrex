...
if($_GET['type'] == "cancelOrder"){
    $cancelOrder = $bit->cancel($_POST['orderNumber']);
}
...
if($_GET['type'] == "getTicker"){
    $ticker = $bit->getTicker($_POST['coin']);
    $ticker->Bid = number_format($ticker->Bid,8);
    $ticker->Ask = number_format($ticker->Ask,8);
    $ticker->Last = number_format($ticker->Last,8);
