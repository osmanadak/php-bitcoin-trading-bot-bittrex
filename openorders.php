                        ...
                        <div class="card-body m-t-0">
                            <table class="table table-inner table-vmiddle">
                                ...
                                for($i=0; $i<count($coins); $i++){
                                    $openOrders = $bit->getOpenOrders($coins[$i]);
                                    if(count($openOrders) > 0 ){
                                        for($k=0; $k<count($openOrders); $k++){
                                            if($openOrders[$k]->OrderUuid != ""){
                                                $j++;
                                                $stmt2 = $dbh->prepare("select * from processes where sellOrderNumber = '".$openOrders[$k]->OrderUuid."'");
                                                $stmt2->execute();
                                                $row = $stmt2->fetch();
                                                $currentPrice = $bit -> getMarketSummary($coins[$i]);
                                                $currentPrice[0]->Last = number_format($currentPrice[0]->Last,8);
                                                $profitStatus = carpmaBtc($openOrders[$k]->Quantity,cikarmaBtc($currentPrice[0]->Last,$row['buyPrice']));
                                    ?>
                                    <tr>
                                        <td class="f-500 c-cyan"><?php echo $j;?></td>
                                        <td><?php echo $coins[$i];?></td>
                                        <td><?php echo $openOrders[$k]->Quantity;?></td>
                                        <td><?php echo $row['buyPrice'];?></td>
                                        <td><?php echo $row['sellPrice'];?></td>
                                        <td><?php echo $currentPrice[0]->Last;?></td>
                                        <td <?php if($profitStatus < 0){echo "style='color: red;'";}else{echo "style='color: green;'";}?>><?php echo $profitStatus;?></td>
                                        <td style="width: 10%;">
                                            <button class="btn btn-danger" onclick="cancelOrder('<?php echo $coins[$i];?>','<?php echo $openOrders[$k]->OrderUuid;?>');">Cancel Order</button>
                                        </td>
                                    </tr>
                                <?php }}}} ?>
                                </tbody>
